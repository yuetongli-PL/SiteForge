...
test('social-auth-recover builds X keepalive, manual login, and verification commands', () => {
  ...
  assert.match(site.commandLines.keepalive, /src\/entrypoints\/sites\/site-keepalive\.mjs/u);
  assert.match(site.commandLines.keepalive, /--no-auto-login/u);
  assert.match(site.commandLines.keepalive, /--user-data-dir/u);
  assert.match(site.commandLines.manualLogin, /src\/entrypoints\/sites\/site-login\.mjs/u);
  ...
  assert.match(site.commandLines.verify[0].command, /scripts\/social-live-verify\.mjs/u);
