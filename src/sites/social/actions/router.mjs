...
function buildDownloadableMediaPlan(media, maxItems = Infinity) {
  const expectedMedia = [];
...
async function downloadOneMediaCandidate({ entry, index, headers, outDir, siteKey, account, contentHashIndex, previousDownloads, skipExisting, retries, retryBackoffMs }) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    try {
      ...
      if (!response.ok) {
        const curlResult = await runCurlDownload({
          url: entry.url,
          headers,
...
function runCurlDownload({ url, headers, filePath }) {
...
export async function runSocialAction(options = {}, deps = {}) {
  ...
  try {
    ...
    if (settings.downloadMedia) {
      ...
      const downloadResult = await downloadMediaFiles({
        media,
        headers,
        outDir: mediaOutDir,
        siteKey: executionPlan.siteKey,
