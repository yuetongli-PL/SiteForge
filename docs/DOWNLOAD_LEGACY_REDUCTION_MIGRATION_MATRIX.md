...
| Douyin | Ordinary video input resolved by direct injected media results or `resolveDouyinMediaBatch` using `douyin-native-resolver-deps-v1`. | Native | `native-douyin-resource-seeds` | `douyin-resource-seeds-provided` | `tests/node/download-douyin-native-resolver.test.mjs` | Keep Douyin legacy action when no direct media or injected resolver result exists. |
