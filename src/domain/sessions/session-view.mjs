...
export const SESSION_VIEW_STATUSES = Object.freeze(['ready', 'blocked', 'manual-required', 'quarantine', 'expired']);
...
const SESSION_VIEW_STATUS_SET = new Set(SESSION_VIEW_STATUSES);
...
function normalizeSessionViewStatus(value) {
  const status = normalizeText(value) ?? 'ready';
  if (!SESSION_VIEW_STATUS_SET.has(status)) {
    throw new Error(`SessionView status ${status} is not supported`);
  }
  return status;
}

function normalizeSessionViewExpiresAt(value) {
  const expiresAt = normalizeText(value);
  if (!expiresAt) {
    return undefined;
  }
  if (!Number.isFinite(Date.parse(expiresAt))) {
    throw new Error('SessionView expiresAt must be an ISO-compatible timestamp');
  }
  return expiresAt;
}

function normalizeEvaluationNow(value) {
  if (value === undefined || value === null) {
    return undefined;
  }
  const timestamp = value instanceof Date ? value.getTime() : Date.parse(String(value));
  if (!Number.isFinite(timestamp)) {
    throw new Error('SessionView now must be an ISO-compatible timestamp');
  }
...
function assertSessionViewAuthorizationState(view = {}, { now } = {}) {
  ...
  if (view.expiresAt && now && Date.parse(view.expiresAt) <= Number(now) && view.status === 'ready') {
    throw new Error('SessionView ready status is expired');
  }
  return true;
}
...
export function normalizeSessionView(raw = {}, context = {}) {
  ...
  const now = normalizeEvaluationNow(context.now);
  ...
  const view = {
    ...
    ttlSeconds,
    expiresAt: normalizeSessionViewExpiresAt(raw.expiresAt),
    networkContext: normalizeObject(raw.networkContext),
    status: normalizeSessionViewStatus(raw.status),
  ...
  assertSessionViewAuthorizationState(view, { now });
