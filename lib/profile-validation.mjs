...
import { resolveLegacyProfileArchetype, resolveProfileArchetype } from './sites/archetypes.mjs';
  ...
  switch (schema.type) {
    ...
    case 'boolean': {
      if (typeof value !== 'boolean') {
        pushError(errors, at, 'must be a boolean');
      }
      break;
    }
    default:
...
export function validateProfileObject(profile, options = {}) {
  ...
  const resolvedArchetype = resolveProfileArchetype(profile, { host: schemaHost }) ?? null;
  if (resolvedArchetype) {
    normalizedProfile.archetype = resolvedArchetype;
  } else if (schemaHost) {
    pushError(errors, 'profile.archetype', `has no registered schema for host ${JSON.stringify(schemaHost)}`);
  } else {
    pushError(errors, 'profile.archetype', 'is required to resolve a schema');
  }

  if (!Object.prototype.hasOwnProperty.call(profile, 'archetype')) {
    const legacyArchetype = resolveLegacyProfileArchetype(schemaHost);
    if (legacyArchetype) {
      warnings.push(`profile.archetype defaulted to ${JSON.stringify(legacyArchetype)} for legacy host ${JSON.stringify(schemaHost)}`);
    }
  }

  if (!Object.prototype.hasOwnProperty.call(profile, 'schemaVersion') && resolvedArchetype) {
    normalizedProfile.schemaVersion = 1;
    warnings.push('profile.schemaVersion defaulted to 1 for compatibility');
  }

  const schema = resolvedArchetype ? resolveProfileSchema(resolvedArchetype) : null;
  if (!schema && resolvedArchetype) {
    pushError(errors, 'profile.archetype', `has no registered schema for ${JSON.stringify(resolvedArchetype)}`);
  }

  if (schema) {
    validateAgainstSchema(normalizedProfile, schema, 'profile', errors);
  }
