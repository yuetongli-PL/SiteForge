...
const GRAPH_LAYER_DESIGN_SOURCE_REFERENCES = Object.freeze([
  Object.freeze({
    path: 'docs/site-capability-layer/DESIGN.md',
    role: 'requested-layer-design-path',
    status: 'missing',
    verified: false,
    note: 'Do not treat the missing Layer design path as present or verified.',
  }),
  Object.freeze({
...
const LAYER_SOURCE_BILIBILI_ADAPTER_REF = 'src/sites/core/adapters/bilibili.mjs';
...
function normalizeGraphMigrationReportRepoOutputTarget(value) {
  const text = assertRequiredText(
    value ?? 'docs/site-capability-graph/generated-migration-report.json',
    'targetRelativePath',
  ...
  if (!/^docs\/site-capability-graph\/[a-z0-9][a-z0-9-]{0,79}\.(?:json|md)$/u.test(text)) {
    throw new Error('GraphMigrationReportRepoOutputDryRun targetRelativePath must be docs/site-capability-graph/<artifact>.json or .md');
  }
...
function normalizeGraphInventoryRepoOutputTarget(value) {
  const text = assertRequiredText(
    value ?? 'docs/site-capability-graph/generated-site-capability-graph.json',
    'targetRelativePath',
  ...
  if (!/^docs\/site-capability-graph\/[a-z0-9][a-z0-9-]{0,79}\.json$/u.test(text)) {
    throw new Error('GraphInventoryRepoOutputDryRun targetRelativePath must be docs/site-capability-graph/<artifact>.json');
  }
  assertNoForbiddenPatterns(text);
  return text;
}
...
function normalizeGraphDocsMarkdownGeneratedOutputManifestTarget(value) {
  const text = assertRequiredText(
    value ?? 'docs/site-capability-graph/generated-docs-summary.manifest.json',
  ...
  if (!/^docs\/site-capability-graph\/[a-z0-9][a-z0-9-]{0,79}\.manifest\.json$/u.test(text)) {
    throw new Error('GraphDocsMarkdownGeneratedOutputManifestGuard manifestRelativePath must be docs/site-capability-graph/<artifact>.manifest.json');
  }
  assertNoForbiddenPatterns(text);
  return text;
}
...
function normalizeGraphDocsMarkdownRetainedOutputIndexTarget(value) {
  const text = assertRequiredText(
    value ?? 'docs/site-capability-graph/generated-docs-summary.retained-index.json',
    'indexRelativePath',
  ...
  if (!/^docs\/site-capability-graph\/[a-z0-9][a-z0-9-]{0,79}\.retained-index\.json$/u.test(text)) {
    throw new Error('GraphDocsMarkdownRetainedOutputIndexGuard indexRelativePath must be docs/site-capability-graph/<artifact>.retained-index.json');
  }
  assertNoForbiddenPatterns(text);
  return text;
}
...
function normalizeGraphDocsMarkdownRepoOutputTarget(value) {
  const text = assertRequiredText(
    value ?? 'docs/site-capability-graph/generated-docs-summary.md',
    'targetRelativePath',
  ...
  if (!/^docs\/site-capability-graph\/[a-z0-9][a-z0-9-]{0,79}\.md$/u.test(text)) {
    throw new Error('GraphDocsMarkdownRepoOutputDryRun targetRelativePath must be docs/site-capability-graph/<artifact>.md');
  }
  assertNoForbiddenPatterns(text);
  return text;
}
