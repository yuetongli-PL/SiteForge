...
| --- | --- | --- | --- | --- |
| Bilibili | `bilibili-dash-mux` | `runs\live-validation\bilibili-dash-mux\20260430T213818_native_BV1WjDDBGE3p\manifest.json` | `passed` | Approved real native DASH run. `resolved-task.json` used `native-bilibili-resource-seeds` with 2 resources, manifest has no `legacy` field, `downloads.jsonl` records derived `reason: dash-mux`, and ffprobe verified H.264 video plus AAC audio in the mux file. |
