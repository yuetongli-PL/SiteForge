...
test('Execution graph nodes preserve destructive governance descriptors', () => {
  const impactScope = {
    material: 'descriptor_only',
    level: 'destructive',
    destructive: true,
    highRisk: true,
    paymentOrFunds: false,
    scopeKinds: ['record_or_data'],
    affectedResourceRefs: ['capability:synthetic.example:delete-record'],
    reversibility: 'irreversible_or_site_defined',
  };
  const executionPrerequisites = {
    material: 'descriptor_only',
    sitePolicyExplicitAllowRequired: true,
    strongConfirmationRequired: true,
    auditRequired: true,
    runtimeConstraintRequired: true,
    sessionRequired: true,
    authRequired: true,
    requiredPolicyFlags: ['allowDestructiveActions'],
  ...
  const confirmationPolicy = {
    ...
    acceptedConfirmationRefs: ['execution_contract_id', 'capability_id'],
    naturalLanguageRequestGrantsExecution: false,
  };
  const auditPolicy = {
    ...
    fields: ['buildId', 'siteId', 'contractRef', 'capabilityId', 'impactScope', 'runtimeDecision', 'resultStatus'],
  };

  assert.equal(assertRuntimeBindingNodeCompatible({
    schemaVersion: GRAPH_NODE_SCHEMA_VERSION,
    id: 'runtime-binding:synthetic.example:delete-record',
    type: 'RuntimeBindingNode',
    siteKey: 'synthetic.example',
    bindingKind: 'site_adapter_or_public_http',
    allowedMaterial: ['schema', 'template', 'slot', 'redacted-descriptor'],
    forbiddenMaterial: ['cookie-value', 'auth-header-value', 'complete-header-map', 'request-response-body'],
    redactionRequired: true,
    credentialMaterialPolicy: 'no_raw_material',
    sourceRefs: ['config/site-capabilities.json'],
  }), true);

  assert.equal(assertGovernancePolicyNodeCompatible({
    schemaVersion: GRAPH_NODE_SCHEMA_VERSION,
    id: 'governance-policy:synthetic.example:delete-record',
    type: 'GovernancePolicyNode',
    executionDisposition: 'blocked',
    executionVerdict: 'blocked',
    executionGates: ['confirm_required', 'audit_required', 'session_required', 'permission_required'],
    auditRequired: true,
    ...
    runtimeConstraintRequired: true,
    naturalLanguageRequestGrantsExecution: false,
    runtimeDispatchAllowedByDefault: false,
    impactScope,
    executionPrerequisites,
    confirmationPolicy,
    auditPolicy,
    reasonCodeRefs: ['execution.destructive_default_blocked'],
    sourceRefs: ['config/site-capabilities.json'],
  }), true);

  assert.equal(assertExecutionContractNodeCompatible({
    schemaVersion: GRAPH_NODE_SCHEMA_VERSION,
    id: 'execution-contract:synthetic.example:delete-record',
    type: 'ExecutionContractNode',
    siteKey: 'synthetic.example',
    capabilityRef: 'capability:synthetic.example:delete-record',
    operationKind: 'form_or_action',
    requestSchemaRef: 'schema:synthetic.example:delete-record:request',
    responseSchemaRef: 'schema:synthetic.example:delete-record:response',
    runtimeBindingRef: 'runtime-binding:synthetic.example:delete-record',
    sessionRequirementRef: 'session-requirement:synthetic.example:delete-record',
    authRequirementRef: 'auth-requirement:synthetic.example:delete-record',
    riskPolicyRef: 'risk-policy:synthetic.example:blocked',
    governancePolicyRef: 'governance-policy:synthetic.example:delete-record',
    executionDisposition: 'blocked',
    executionVerdict: 'blocked',
    executionGates: ['confirm_required', 'audit_required', 'session_required', 'permission_required'],
