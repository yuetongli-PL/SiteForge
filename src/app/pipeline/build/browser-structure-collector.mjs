...
export function browserStructureCollectorScript({
  nonce = '__SITEFORGE_NONCE__',
  submitUrl = '__SITEFORGE_SUBMIT_URL__',
  sourceLayer = 'authenticated',
} = /** @type {any} */ ({})) {
  return `(() => {
  const SITEFORGE_NONCE = ${JSON.stringify(String(nonce ?? ''))};
  const SITEFORGE_SUBMIT_URL = ${JSON.stringify(String(submitUrl ?? ''))};
  const SITEFORGE_SOURCE_LAYER = ${JSON.stringify(String(sourceLayer ?? 'authenticated'))};
  const MAX_LINKS = 160;
  ...
  const semanticKindFor = (href, label, node) => {
    ...
    if (/author|profile|user|org|organization|people|actor|model|star|\\u4f5c\\u8005|\\u7528\\u6237|\\u4e3b\\u9875/u.test(text)) return 'profile';
    if (/detail|item|product|content|\\u8be6\\u60c5|\\u76ee\\u5f55|\\u4e66\\u9875/u.test(text)) return 'detail';
    return 'navigation';
  };
  ...
  const links = [...document.querySelectorAll('a[href], area[href]')]
    .filter(visible)
    .map((node, index) => {
      const href = sameOriginUrl(hrefOf(node));
      if (!href) return null;
  ...
  const controls = [...document.querySelectorAll('button, input, select, textarea, [role="button"], [role="tab"], [role="menuitem"]')]
    .filter(visible)
    .map((node, index) => ({
      kind: String(node.tagName || '').toLowerCase() === 'button' ? 'button' : String(node.tagName || 'control').toLowerCase(),
  ...
  const payload = SITEFORGE_SOURCE_LAYER === 'authenticated_overlay'
    ? { nonce: SITEFORGE_NONCE, authenticatedOverlayPages: [page] }
    : { nonce: SITEFORGE_NONCE, authenticatedPages: [page] };
  return fetch(SITEFORGE_SUBMIT_URL, {
    method: 'POST',
