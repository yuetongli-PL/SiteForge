...
- Current gaps: Actual evidence score still depends on live/governed capture breadth; FullDiscoveryMode records unknown/blocked/skipped/unattempted/failed/duplicate gaps rather than claiming unobserved surfaces do not exist. The controlled-scope artifact closure loop reached 100/100/100/100 with six-agent acceptance, but that score does not claim live/full-web exhaustive crawling.
- Next smallest task: Add actual governed trigger retry attempt fixtures and direct producer evidence without bypassing policy or saving raw payloads.
...
- Current gaps: Verified live API evidence remains site-specific and must be explicitly recorded; actual governed trigger retry evidence and verified API catalog promotion remain separate tasks. The controlled-scope API closure now accounts for current artifact families without treating observed APIs as verified catalog entries, and the final controlled-scope review accepted API discovery as 100 for artifact closure only.
- Next smallest task: Add verified API catalog promotion evidence behind adapter/policy/schema/test gates.
